...
document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            if (diff === 1) {
                ...
                if (sB === 27) {
                    ...
                    const digits9 = []; const groups = [];
                    for (let i = 0; i < digits3.length; i += 2) {
                        const val = digits3[i] * 3 + digits3[i + 1];
                        digits9.push(val);
    ...
    // --- EVENT LISTENERS ---
    dom.inputValue.addEventListener('input', (e) => state.inputValue = e.target.value);
    ...
    const init = () => {
        ...
        dom.sourceBase.value = state.sourceBase;
        dom.targetBase.value = state.targetBase;
